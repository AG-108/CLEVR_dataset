...
def set_layer(obj, original_layer_idx, layer_idx):
  """ Move an object to a particular layer """
  # Set the target layer to True first because an object must always be on
  # at least one layer.
  coll = bpy.data.collections.get(f'Collection_{layer_idx}')
  original_coll = bpy.data.collections.get(f'Collection_{original_layer_idx}')
  if (not original_coll is None) and original_coll != -1:
    original_coll.objects.unlink(obj)
  if coll is None:
    coll = bpy.data.collections.new(f'Collection_{layer_idx}')
  if not obj.name in coll.objects:
    coll.objects.link(obj)
  # obj.cycles_visibility[layer_idx] = True
  # for i in range(len(obj.layers)):
  #   obj.cycles_visibility[i] = (i == layer_idx)
